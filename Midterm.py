...
# 手部判斷 需要SVM 回傳預測結果
def reconHand(img, svm, voc):
	img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
	img = cv2.resize(img, (500, 500))
	sift = cv2.SIFT_create()
	kp, des = sift.detectAndCompute(img, None)
	im_feature = np.zeros((1, 40), "float32")
	words, distance = vq(des, voc)
	for w in words:
		im_feature[w] += 1
	result = svm.predict(im_feature)
	return result
...
def RockPaperScissors(voc):
	...
	while(userHand == 0):
		...
		userHand = reconHand(userImg, svm, voc)		# 玩家的手 整數
		if userHand == 0:
			print("玩家手勢錯誤! 請在試一次")
		else:
